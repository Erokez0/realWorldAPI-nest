...
export class CommentsController {
  ...
  async addComment(
    ...
    const isFollowing = this.userService.isFollowing(
      user.userId,
      comment.authorId,
    )
  ...
  async getComments(@Param('slug') slug: string, @User() user: ReqUser) {
    const comments = await this.commentsService.findAll(slug)
    return comments.map((comment) => {
      const isFollowing = user
        ? this.userService.isFollowing(user.userId, comment.authorId)
        : false
      return new CommentEntity(comment, isFollowing)
    })
  }
