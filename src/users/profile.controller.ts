...
export class ProfileController {
	...
	async findOne(@Param('username') username: string, @User() user: ReqUser) {
		...
		const isFollowing = user ? this.usersService.isFollowing(user.userId, profile.id) : false;
	...
	async follow(@Param('username') username: string, @User() user: ReqUser) {
		return new ProfileEntity(await this.usersService.follow(username, user.userId), true)
	}
	...
	async unfollow(@Param('username') username: string, @User() user: ReqUser) {
		return new ProfileEntity(await this.usersService.unfollow(username, user.userId), false)
	}
}
